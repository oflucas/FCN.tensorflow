...
def main(train_dir, test_dir, outputDir):
    ...
    for cat in dataStructure.cats:
        ...
        for trainData_fileName_gt in trainData_fileList_gt:
            ...
            full_gt = cv2.imread(trainData_fileName_gt, cv2.IMREAD_UNCHANGED) #cv2.CV_LOAD_IMAGE_UNCHANGED)
            ...
            shape_dict[trainData_file_gt.shape] = trainData_fileName_gt

        # Compute prop
        locationPotential = locationPotential/len(trainData_fileList_gt)
        locationPotential_uinit8 = (locationPotential*255).astype('u1')
        
        print 'gt image shape:', cat, shape_dict
        print 'trainData_file_gt:', trainData_file_gt.shape, 'out:', locationPotential_uinit8.shape
